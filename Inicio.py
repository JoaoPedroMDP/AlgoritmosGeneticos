...
def plot_run(data):
    ...
    fig = go.Figure(data=[go.Surface(z=z_errors, name="Erros"), go.Surface(z=z_weights, name="Pesos", opacity=0.7)])
    fig.update_layout(
        uniformtext_minsize=18, uniformtext_mode='hide',
...
def app():
    ...
    for i in range(round_count + 1):
        round_name = i if i < round_count else "Bônus"
        with st.expander(f"## Rodada {round_name}"):
            if i == round_count:
                st.write("Nesta rodada, a população inicial é composta pelos melhores indivíduos de todas as rodadas anteriores")
            ...
            if i == round_count and config['mutation_percent_genes'] == 0:
                config['mutation_type'] = None
