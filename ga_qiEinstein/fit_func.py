...
WEIGHTS: dict = {1: 9, 2: 6, 3: 4, 4: 5, 5: 7, 6: 7, 7: 2, 8: 6, 9: 9, 10: 1, 11: 1, 12: 3, 13: 4, 14: 8, 15: 2}
...
def add_error_occurrence(rule_index: int, error_occurrences: dict):
    error_occurrences[0] = error_occurrences.get(0, 0) + 1

    if rule_index not in error_occurrences:
        error_occurrences[rule_index] = 0
    error_occurrences[rule_index] += 1
...
def fit(bitline: np.ndarray, error_occurrences: dict = None):
    ...
    if l_error > 0 and error_occurrences is not None:
        add_error_occurrence(rule, error_occurrences)
    error += l_error
